...
namespace KQPBO
{
	...
	QPBO::QPBO(int node_num_max, int edge_num_max, void (*err_function)(char *))
	: node_num(0),
	  nodeptr_block(NULL),
	  changed_list(NULL),
	  ...
	  zero_energy(0)
{
	...
	if (!nodes[0] || !arcs[0]) { if (error_function) (*error_function)("Not enough memory!"); assert(false); }
	...
	void QPBO::Reset()
{
	...
	void QPBO::reallocate_nodes(int node_num_max_new)
{
	...
	if (!nodes[0]) { if (error_function) (*error_function)("Not enough memory!"); assert(false); }
	...
	void QPBO::reallocate_arcs(int arc_num_max_new)
{
	...
	if (!arcs[0]) { if (error_function) (*error_function)("Not enough memory!"); assert(false); }
...
/////////////////////////////////////////////////////////////////////////
	...
	bool QPBO::Save(char* filename, int format)
{
	int e;
	int edge_num = 0;
	...
	bool QPBO::Load(char* filename)
{
	...
	sprintf_s(FORMAT_LINE_NODE, "n %%d %%%s %%%s\n", type_format, type_format);
	sprintf_s(FORMAT_LINE_EDGE, "e %%d %%d %%%s %%%s %%%s %%%s\n", type_format, type_format, type_format, type_format);
	while (fgets(LINE, sizeof(LINE), fp))
	{
		if (sscanf_s(LINE, FORMAT_LINE_EDGE, &i, &j, &E00, &E01, &E10, &E11) == 6)
		{
			if (i<0 || i>=NODE_NUM || j<0 || j>=NODE_NUM || i==j) { printf("%s: wrong format\n", filename); fclose(fp); return false; }
			AddPairwiseTerm(i, j, E00, E01, E10, E11);
		}
		else if (sscanf_s(LINE, FORMAT_LINE_NODE, &i, &E0, &E1) == 3)
		{
...
#define SET_TO(a, j)             (a)->head = (j);
	...
	QPBO::EdgeId QPBO::AddPairwiseTerm(NodeId _i, NodeId _j, REAL E00, REAL E01, REAL E10, REAL E11)
{
	...
	void QPBO::AddPairwiseTerm(EdgeId e, NodeId _i, NodeId _j, REAL E00, REAL E01, REAL E10, REAL E11)
{
	...
	void QPBO::TransformToSecondStage(bool copy_trees)
{
	...
	void QPBO::Solve()
{
	...
	REAL QPBO::ComputeTwiceEnergy(int option)
{
	...
	REAL QPBO::ComputeTwiceEnergy(int* solution)
{
	REAL E = 2*zero_energy, E1[2], E2[2][2];
	int i, j, e;
	...
	REAL QPBO::ComputeTwiceLowerBound()
{
