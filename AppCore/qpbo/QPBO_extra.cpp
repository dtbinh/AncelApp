...
/////////////////////////////////////////////////////////////////////////
	...
	void QPBO::ComputeRandomPermutation(int N, int* permutation)
{
	int i, j, k;
	for (i=0; i<N; i++)
	{
	...
	void QPBO::MergeMappings(int nodeNum0, int* mapping0, int* mapping1)
{
	int i;
...
#define SET_TO(a, j)             (a)->head = (j);
	...
	inline void QPBO::FixNode(Node* i, int x)
{
	...
	inline void QPBO::ContractNodes(Node* i, Node* j, int swap)
{
	...
	int QPBO::MergeParallelEdges(Arc* a1, Arc* a2)
{
	...
	inline REAL QPBO::DetermineSaturation(Node* i)
{
	Arc* a;
	REAL c1 = -i->tr_cap;
	REAL c2 = i->tr_cap;
	...
	inline void QPBO::AddDirectedConstraint(Node* i, Node* j, int xi, int xj)
{
...
/*
	...
	inline bool QPBO::AddDirectedConstraint(Arc* a, int xi, int xj)
{
...
*/
	...
	inline bool QPBO::AddDirectedConstraint0(Arc* a, int xi, int xj)
{
	Node* i = a->sister->head;
	Node* j = a->head;
	Node* _i[2] = { i, GetMate0(i) };
	...
	inline bool QPBO::AddDirectedConstraint1(Arc* a, int xi, int xj)
{
	Node* j = a->head;
	Node* _j[2];
	Arc* _a[2] = { a, GetMate(a) };
	...
	void QPBO::AllocateNewEnergy(int* mapping)
{
	int i_index, j_index;
...
private:
	...
	void QPBO::SetMaxEdgeNum(int num)
{
	if (num > GetMaxEdgeNum()) reallocate_arcs(2*num);
}


	bool QPBO::Probe(int* mapping)
{
	...
	void QPBO::Probe(int* mapping, ProbeOptions& options)
{
	...
	bool QPBO::Improve(int N, int* order_array, int* fixed_nodes)
{
	int p, i_index;
	Node* i;
	Node* _i[2];
